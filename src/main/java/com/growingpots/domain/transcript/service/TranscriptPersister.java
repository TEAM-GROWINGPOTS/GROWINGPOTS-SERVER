...
public class TranscriptPersister {
    ...
    private List<StudentMajor> saveMajorsAndSummaries(
        ...
        deleteStaleStudentMajors(studentProfile, studentMajors);
    ...
    private void deleteStaleStudentMajors(StudentProfile studentProfile, List<StudentMajor> currentMajors) {
        Set<Long> currentMajorIds = currentMajors.stream().map(StudentMajor::getId).collect(Collectors.toSet());
        List<StudentMajor> existingMajors = studentMajorRepository.findWithDepartmentByStudentProfile(studentProfile);
