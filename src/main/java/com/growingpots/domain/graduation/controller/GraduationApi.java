...
public @interface GraduationApi {
    ...
    @Operation(
            ...
            description = "PDF 스냅샷 기반 졸업 현황을 반환한다. "
                    ...
                    + "DISTRIBUTED_GE satisfied: 24학번 이상은 학점 충족 AND 5개 영역 중 3개 이상 이수를 모두 충족해야 true. "
                    + "19~23학번은 학점 기준만 적용(기존과 동일). "
    ...
    @Operation(
            ...
            description = "divisionCode(예: MAJOR_REQUIRED)에 해당하는 이수/미이수 과목 목록을 반환한다. "
                    ...
                    + "DISTRIBUTED_GE: 24학번 이상이면 areaRequirement 블록이 채워진다(5개 영역 완료 현황). "
                    + "satisfied는 학점 충족 AND 3개 이상 영역 이수를 모두 만족해야 true. "
                    + "19~23학번은 areaRequirement=null, satisfied는 학점 기준만 적용. "
                    + "각 과목 카드의 area 필드에 해당 과목의 배분이수 영역 정보(코드·이름)가 담긴다(영역 미지정 과목은 null). "
